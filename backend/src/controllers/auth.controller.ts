...
import { generateAccessToken, generateRefreshToken, verifyToken, TokenPayload } from '../middleware/auth.middleware.js';
  ...
  try {
    ...
    if (req.tenant) {
      ...
      const adminResult = await centralPool.query(
        `SELECT ta.id, ta.email, ta.first_name, ta.last_name, ta.phone, ta.password, ta.password_hash, ta.is_primary,
                t.id as tenant_id, t.subdomain
         FROM tenant_admins ta
         JOIN tenants t ON ta.tenant_id = t.id
         WHERE ta.email = $1 AND t.id = $2 AND t.status != 'deleted'`,
        [email, req.tenant.id]
      );
...
export const getCurrentUser = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user.isTenantAdmin) {
      const adminResult = await centralPool.query(
        `SELECT ta.id, ta.email, ta.first_name, ta.last_name, ta.phone, ta.is_primary
         FROM tenant_admins ta
         WHERE ta.id = $1`,
        [req.user.id]
      );
