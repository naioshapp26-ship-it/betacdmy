...
type TenantSummaryRow = {
  ...
  primary_admin_first_name: string | null;
  primary_admin_last_name: string | null;
  billing_plan: string | null;
  billing_status: string | null;
  billing_price_monthly: string | null;
...
const mapTenantSummary = (row: TenantSummaryRow) => {
...
const fetchTenantSummary = async (id: string) => {
  const result = await centralPool.query<TenantSummaryRow>(`${tenantSummarySelect} WHERE t.id = $1 LIMIT 1`, [id]);
  return result.rows[0] ? mapTenantSummary(result.rows[0]) : null;
};

const fetchTenantRecord = async (id: string): Promise<TenantRow | null> => {
  const result = await centralPool.query<TenantRow>(`SELECT * FROM tenants WHERE id = $1 LIMIT 1`, [id]);
  return result.rows[0] ?? null;
};

export const createSuperAdminRouter = (deps: RouterDeps = {}) => {
  const router = Router();
  ...
  router.get('/api/super-admin/tenants', async (_req, res) => {
    try {
      const result = await centralPool.query<TenantSummaryRow>(`${tenantSummarySelect} ORDER BY t.created_at DESC`);
  ...
  router.get('/api/super-admin/seo/settings', requireAuth, async (req, res) => {
    if (!(await isAdmin(req))) {
      return res.status(403).json(
        createErrorResponse('errors.forbidden', req, 'Admin access required')
      );
    }
    ...
    try {
      ...
      if (!result.rowCount) {
        return res.status(404).json(
          createErrorResponse('errors.notFound', req, 'SEO setting not found')
        );
      }
  ...
  router.post('/api/super-admin/seo/settings', requireAuth, async (req, res) => {
    ...
    if (!page_path) {
      return res.status(400).json(
        createErrorResponse('errors.validation', req, 'Page path is required')
      );
    }

    try {
      const existingCheck = await centralPool.query(
        'SELECT id FROM central_seo_settings WHERE page_path = $1',
        [page_path]
      );

      if (existingCheck.rowCount) {
        return res.status(409).json(
          createErrorResponse('errors.conflict', req, 'SEO setting for this page path already exists')
        );
      }
  ...
  router.put('/api/super-admin/seo/settings/:id', requireAuth, async (req, res) => {
    ...
    try {
      const existingCheck = await centralPool.query(
        'SELECT id FROM central_seo_settings WHERE id = $1',
        [id]
      );
      ...
      if (page_path) {
        const conflictCheck = await centralPool.query(
          'SELECT id FROM central_seo_settings WHERE page_path = $1 AND id != $2',
          [page_path, id]
        );

        if (conflictCheck.rowCount) {
          return res.status(409).json(
            createErrorResponse('errors.conflict', req, 'Another SEO setting with this page path already exists')
          );
        }
      }
  ...
  router.get('/api/seo/settings/page', async (req, res, next) => {
    ...
    if (!path) {
      return res.status(400).json(
        createErrorResponse('errors.validation', req, 'Path query parameter is required')
      );
    }
      ...
      if (payload.primaryAdminEmail) {
        ...
        if (email) {
          await centralPool.query(
            `INSERT INTO tenant_admins (tenant_id, email, is_primary)
             VALUES ($1, $2, true)
             ON CONFLICT (tenant_id, email) DO UPDATE SET is_primary = true` ,
            [id, email]
          );
  ...
  router.get('/api/super-admin/provisioning-logs', async (req, res) => {
    ...
    try {
      const result = await centralPool.query(
        `SELECT id, tenant_id, subdomain, status, step, message, started_at, completed_at
  ...
  router.get('/api/super-admin/analytics', async (_req, res) => {
    try {
      const [tenantCounts, paymentSummary] = await Promise.all([
        ...
        centralPool.query(
          `SELECT COALESCE(SUM(amount), 0) AS total_revenue, COUNT(*) AS payment_events FROM payment_transactions`
        )
      ]);
  ...
  router.post('/api/super-admin/payments/:paymentId/refund', async (req, res) => {
    ...
    const adminEmail = (req as any).adminEmail || (req as any).user?.email || 'super-admin';
    const adminName = (req as any).adminName || (req as any).user?.name || 'Super Admin';
  ...
  router.get('/api/super-admin/refunds', async (req, res) => {
    const status = req.query.status as 'pending' | 'succeeded' | 'failed' | undefined;
    const limit = Math.min(Number(req.query.limit) || 50, 200);
