...
export const createPaymentWebhookRouter = (provisioning = new ProvisioningService()) => {
   ...
   */
  router.post('/api/webhooks/payment/stripe', webhookRateLimiter, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { eventRecord, isDuplicate } = await webhookService.storeWebhookEvent(
        'stripe',
        event.id,
        event.type,
        event
   ...
   */
  async function processStripeWebhook(event: Stripe.Event, webhookEventId: string): Promise<void> {
      ...
      try {
        if (session.payment_status === 'paid') {
          ...
          await provisioning['central'].query(
            `UPDATE tenants SET status = 'active', updated_at = NOW() WHERE id = $1 AND status = 'pending_payment'`,
            [tenantId]
          );
   ...
   */
  router.post('/api/webhooks/payment/paypal', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { eventRecord, isDuplicate } = await webhookService.storeWebhookEvent(
        'paypal',
        paymentData.id || `paypal-${Date.now()}`,
        eventType || 'UNKNOWN',
        paymentData
      );
      ...
      if (eventType === 'CHECKOUT.ORDER.APPROVED' || eventType === 'PAYMENT.CAPTURE.COMPLETED') {
        ...
        if (customId) {
          // Activate the tenant
          await provisioning['central'].query(
            `UPDATE tenants SET status = 'active', activated_at = NOW() WHERE id = $1 AND status = 'pending_payment'`,
            [customId]
          );
