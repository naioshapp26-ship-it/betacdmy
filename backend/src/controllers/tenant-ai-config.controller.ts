...
`;
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const tenantPool = (req as any).tenantPool;
        const context = tenantPool ? { type: 'tenant' as const, tenantPool } : { type: 'central' as const };
        ...
        if (!publicConfig.aiEnabled || !apiKey) {
          return res.status(404).json(
            createErrorResponse(
              ...
              req,
              tenantPool ? 'AI not configured for this tenant' : 'Platform AI not configured'
            )
          );
        }
      ...
      try {
        ...
        if (!topic || typeof topic !== 'string') {
          return res.status(400).json(
            createErrorResponse('errors.invalidRequest', req, 'Topic is required')
          );
        }
        ...
        if (normalizeAIProvider(publicConfig.aiProvider) !== 'gemini') {
          return res.status(400).json(
            createErrorResponse('errors.unsupportedProvider', req, 'AI provider not supported for course generation')
          );
        }
...
`;
        ...
        if (!generated) {
          return res.status(500).json(
            createErrorResponse('errors.aiGenerationFailed', req, 'Unable to parse AI response')
          );
        }
              ...
              if (item.type === 'TEXT') {
                ...
                if (!existing || existing.length < 120) {
                  ...
                  const lessonContent = await generateLessonContent(
                    ai,
                    model,
                    publicConfig.temperature,
                    publicConfig.maxTokens,
                    courseTitle,
                    courseDescription,
                    level,
                    module.title || 'Module',
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (maxTokens !== undefined && (maxTokens < 1 || maxTokens > 100000)) {
          return res.status(400).json(
            createErrorResponse('errors.invalidMaxTokens', req, 'Max tokens must be between 1 and 100000')
          );
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (normalizedProvider === 'gemini') {
          ...
          res.status(400).json(
            createErrorResponse('errors.unsupportedProvider', req, `Provider ${normalizedProvider} is not supported`)
          );
        }
      } catch (error: any) {
        console.error('[Tenant Admin] AI test failed:', error);
        res.status(400).json(
          createErrorResponse('errors.aiTestFailed', req, error.message || 'Invalid AI credentials')
        );
      }
    }
  );
