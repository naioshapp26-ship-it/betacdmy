...
type ResetTarget =
  | { type: 'tenant-admin'; id: string; email: string; tenantId: string }
  | { type: 'tenant-user'; id: string; email: string }
  | { type: 'platform-user'; id: string; email: string };

const getEffectiveHost = (req: Request): string => {
  ...
  return req.headers.host || process.env.MAIN_DOMAIN || 'betacdmy.com';
};
...
const findForgotPasswordTarget = async (req: Request, email: string): Promise<ResetTarget | null> => {
  if (req.tenant) {
    const tenantAdminResult = await centralPool.query(
    ...
    if (req.tenantPool) {
      const tenantUserResult = await req.tenantPool.query(
        `SELECT id, email
           FROM users
  ...
  const platformUserResult = await centralPool.query(
    `SELECT id, email
       FROM users
      WHERE LOWER(email) = LOWER($1)
      LIMIT 1`,
...
const persistResetToken = async (req: Request, target: ResetTarget, tokenHash: string, expiresAt: Date) => {
  if (target.type === 'tenant-admin') {
    await centralPool.query(
      `UPDATE tenant_admins
        ...
        WHERE id = $3
          AND tenant_id = $4`,
      [tokenHash, expiresAt.toISOString(), target.id, target.tenantId]
    );
    return;
  }
    ...
    await req.tenantPool.query(
      `UPDATE users
          SET reset_token_hash = $1,
              reset_token_expires = $2
        WHERE id = $3`,
      [tokenHash, expiresAt.toISOString(), target.id]
    );
    return;
  }
  ...
  try {
    ...
    if (!email) {
      return res.status(400).json(
        createErrorResponse('errors.validationRequired', req, 'Email is required')
      );
    }
    ...
    return res.status(500).json(
      createErrorResponse('errors.apiServerError', req, 'Failed to process forgot password request')
    );
  }
};

type ResetRecord =
  | { type: 'tenant-admin'; id: string; tenantId: string }
  | { type: 'tenant-user'; id: string }
  | { type: 'platform-user'; id: string };

const findResetRecordByToken = async (req: Request, tokenHash: string): Promise<ResetRecord | null> => {
  if (req.tenant) {
...
const applyNewPassword = async (req: Request, record: ResetRecord, passwordHash: string) => {
  if (record.type === 'tenant-admin') {
    await centralPool.query(
      `UPDATE tenant_admins
        ...
        WHERE id = $2
          AND tenant_id = $3`,
      [passwordHash, record.id, record.tenantId]
    );
    return;
  }
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!token || !newPassword) {
      return res.status(400).json(
        createErrorResponse('errors.validationRequired', req, 'Token and new password are required')
      );
    }
    ...
    if (!record) {
      return res.status(400).json(
        createErrorResponse('errors.authInvalidToken', req, 'Invalid or expired reset token')
      );
    }
    ...
    return res.status(500).json(
      createErrorResponse('errors.apiServerError', req, 'Failed to reset password')
    );
  }
};
