...
import { requirePermission, isAdmin } from '../middleware/rbac.middleware.js';
...
export const createSEORouter = () => {
  const router = Router();
  const schemaEnsuredPools = new WeakSet<object>();

  const ensureSeoSchema = async (tenantPool: any) => {
  ...
  const resolveTenantUserId = async (tenantPool: any, userId?: string | number | null) => {
    if (!userId) {
      return null;
    }
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to fetch SEO settings')
        );
      }
    }
  );
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to fetch SEO setting')
        );
      }
    }
  );
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const existingCheck = await tenantPool.query(
          'SELECT id FROM seo_settings WHERE page_path = $1',
          [page_path]
        );
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to create SEO setting')
        );
      }
    }
  );
      ...
      try {
        ...
        if (page_path) {
          const conflictCheck = await tenantPool.query(
            'SELECT id FROM seo_settings WHERE page_path = $1 AND id != $2',
            [page_path, id]
          );
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to update SEO setting')
        );
      }
    }
  );
  ...
  router.delete(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to delete SEO setting')
        );
      }
    }
  );
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        res.status(500).json(
          createErrorResponse('errors.apiServerError', req, 'Failed to fetch page SEO settings')
        );
      }
    }
  );
