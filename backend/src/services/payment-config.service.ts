 ...
 * Manages payment gateway credentials for both central (Super Admin) and tenant contexts
 */
...
export class PaymentConfigService {
   ...
   */
  async getPaymentConfig(context: PaymentContext): Promise<PaymentGatewayConfig> {
      ...
      if (row.stripe_secret_key) {
        try {
          ...
          if (Buffer.isBuffer(encryptedData)) {
            row.stripe_secret_key = decryptField(encryptedData, ENCRYPTION_KEY);
      ...
      if (row.stripe_webhook_secret) {
        try {
          if (Buffer.isBuffer(row.stripe_webhook_secret)) {
            row.stripe_webhook_secret = decryptField(row.stripe_webhook_secret, ENCRYPTION_KEY);
      ...
      if (row.visa_secret_key) {
        try {
          if (Buffer.isBuffer(row.visa_secret_key)) {
            row.visa_secret_key = decryptField(row.visa_secret_key, ENCRYPTION_KEY);
          }
        } catch (err: any) {
   ...
   */
  async getCentralPaymentConfig(): Promise<PaymentGatewayConfig> {
   ...
   */
  async getTenantPaymentConfig(tenantPool: Pool): Promise<PaymentGatewayConfig> {
  ...
  async updatePaymentConfig(
    ...
    if (config.stripeSecretKey !== undefined) {
      ...
      values.push(
        config.stripeSecretKey ? encryptField(config.stripeSecretKey, ENCRYPTION_KEY) : null
      );
    }
    ...
    if (config.stripeWebhookSecret !== undefined) {
      ...
      values.push(
        config.stripeWebhookSecret ? encryptField(config.stripeWebhookSecret, ENCRYPTION_KEY) : null
      );
    }
    ...
    if (config.paypalSecretKey !== undefined) {
      updates.push(`paypal_secret_key = $${paramIndex++}`);
      values.push(
        config.paypalSecretKey ? encryptField(config.paypalSecretKey, ENCRYPTION_KEY) : null
      );
    }
    ...
    if (config.visaSecretKey !== undefined) {
      updates.push(`visa_secret_key = $${paramIndex++}`);
      values.push(config.visaSecretKey ? encryptField(config.visaSecretKey, ENCRYPTION_KEY) : null);
    }
   ...
   */
  async getPublicPaymentConfig(context: PaymentContext): Promise<{
    stripeEnabled: boolean;
    stripePublicKey: string | null;
    stripePriceBasicMonthly: string | null;
    stripePriceBasicYearly: string | null;
   ...
   */
  private getEmptyConfig(): PaymentGatewayConfig {
    return {
      stripeEnabled: false,
