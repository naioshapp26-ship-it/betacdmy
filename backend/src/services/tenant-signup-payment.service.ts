...
export interface TenantSignupCheckoutRequest {
  tenantId: string;
...
export class TenantSignupPaymentService {
  ...
  private static readonly TRIAL_PERIOD_DAYS = 14;
   ...
   */
  async createCheckoutSession(request: TenantSignupCheckoutRequest): Promise<CheckoutResponse> {
  ...
  async confirmStripeCheckoutSession(params: {
    tenantId: string;
    sessionId: string;
  }): Promise<{ status: 'paid' | 'unpaid'; transactionId?: string }> {
    ...
    const stripe = new Stripe(config.stripeSecretKey, { apiVersion: '2025-12-15.clover' as any });
   ...
   */
  private getPlanAmount(config: any, plan: string, billingCycle: string): number | null {
   ...
   */
  private getPlanCurrency(config: any, plan: string, billingCycle: string): string | null {
    const key = `plan${this.capitalize(plan)}${this.capitalize(billingCycle)}Currency`;
    return config[key] || null;
  }

  private getStripePriceId(config: any, plan: string, billingCycle: string): string | null {
  ...
  private async buildTenantMetadata(
    tenantId: string,
    customerEmail: string,
    subdomain: string,
    plan: string,
    ...
    try {
      const tenantResult = await centralPool.query(
        `SELECT id, subdomain, company_name, subscription_plan, status, created_at
      ...
      console.warn('[TenantSignupPayment] Failed to build tenant metadata:', (error as Error).message);
      const fallbackEntries: Array<[string, unknown]> = [
        ['tenant_id', tenantId],
   ...
   */
  async getAvailableGateways(): Promise<Array<{ gateway: PaymentGateway; label: string }>> {
    try {
      const config = await this.paymentConfigService.getCentralPaymentConfig();
      const gateways: Array<{ gateway: PaymentGateway; label: string }> = [];
