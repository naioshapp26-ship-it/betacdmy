...
export class WebhookService {
   ...
   */
  async verifyStripeWebhook(rawBody: string | Buffer, signature: string): Promise<Stripe.Event> {
      ...
      try {
        ...
        if (tenantResult.rows.length === 0) {
          ...
          if (tenantPaymentConfig.stripeEnabled && tenantPaymentConfig.stripeSecretKey && tenantPaymentConfig.stripeWebhookSecret) {
            ...
            const event = tenantStripe.webhooks.constructEvent(
              rawBody, 
              signature, 
              tenantPaymentConfig.stripeWebhookSecret
            );
  ...
  async storeWebhookEvent(
    ...
    eventType: string,
    payload: any
  ): Promise<{ eventRecord: WebhookEvent; isDuplicate: boolean }> {
   ...
   */
  async markEventProcessing(id: string): Promise<void> {
   ...
   */
  async getWebhookEventByExternalId(provider: string, eventId: string): Promise<WebhookEvent | null> {
    const result = await centralPool.query(
      `SELECT * FROM webhook_events WHERE provider = $1 AND event_id = $2`,
      [provider, eventId]
    );
   ...
   */
  async getFailedEvents(maxRetries: number = 5): Promise<WebhookEvent[]> {
    const result = await centralPool.query(
   ...
   * Ensure webhook_events table exists
   */
  private async ensureWebhookEventsTable(): Promise<void> {
    await centralPool.query(`
