...
type ProvisioningLogStatus = 'pending' | 'running' | 'success' | 'failed';

type ProvisioningContext = {
  subdomain: string;
  tenantId?: string;
  completedSteps: Set<ProvisioningStep>;
...
// Provisioning state for idempotency and resume
export type ProvisioningState = {
  tenantId: string;
  subdomain: string;
  lastCompletedStep: ProvisioningStep | null;
...
const STEP_ATOMICITY = {
  CREATE_TENANT_RECORD: 'ATOMIC_IN_CENTRAL' as const,
  CREATE_TENANT_DATABASE: 'NON_ATOMIC' as const, // External DB creation
  STORE_DATABASE_SECRET: 'ATOMIC_IN_CENTRAL' as const,
  RUN_MIGRATIONS: 'ATOMIC_PER_TENANT' as const, // Each migration file could be wrapped in transaction
  SEED_DEFAULTS: 'ATOMIC_PER_TENANT' as const,
  CREATE_SUBSCRIPTION: 'ATOMIC_IN_CENTRAL' as const, // Central DB transaction
  CREATE_ADMIN: 'ATOMIC_PER_TENANT' as const,
  SEND_WELCOME_EMAIL: 'NON_ATOMIC' as const // External service
};

const normalizeSubdomain = (value: string) => value.trim().toLowerCase();
const formatDatabaseName = (value: string) =>
  `tenant_${value.replace(/[^a-z0-9]/gi, '_').toLowerCase()}`.replace(/_+/g, '_').replace(/^_+|_+$/g, '');
...
export class ProvisioningService {
   ...
   */
  async getProvisioningState(tenantIdOrSubdomain: string): Promise<ProvisioningState | null> {
    ...
    const tenantResult = await this.central.query<TenantRow>(tenantQuery, [tenantIdOrSubdomain]);
    ...
    const successfulSteps = logs.filter(l => l.status === 'success').map(l => l.step as ProvisioningStep);
    const failedSteps = logs.filter(l => l.status === 'failed');
    const runningSteps = logs.filter(l => l.status === 'running');

    let status: ProvisioningState['status'] = 'in_progress';
    let canResume = false;
    ...
    const failedStep = failedSteps.length > 0 
      ? failedSteps[failedSteps.length - 1].step as ProvisioningStep 
      : null;
   ...
   */
  async rollbackProvisioning(tenantId: string, options: { dropDatabase?: boolean; reason?: string } = {}): Promise<void> {
    ...
    try {
      ...
      await this.logStep(context, 'CREATE_TENANT_RECORD', 'failed', 
        `Rollback initiated: ${options.reason || 'manual rollback'}`, 
        { rollback: true, dropDatabase: options.dropDatabase }
      );
      ...
      await this.central.query(
        `UPDATE tenants SET status = 'suspended', suspended_at = NOW(), 
         ...
         WHERE id = $1`,
        [tenantId, JSON.stringify(options.reason || 'provisioning failed')]
      );
   ...
   */
  async resumeProvisioning(tenantId: string, payload?: Partial<CreateTenantInput>): Promise<TenantRow> {
    ...
    const resumePayload: CreateTenantInput = {
      subdomain: tenant.subdomain,
  ...
  async isSubdomainAvailable(subdomain: string): Promise<boolean> {
    ...
    const result = await this.central.query<{ exists: boolean }>(
      `SELECT EXISTS(SELECT 1 FROM tenants WHERE subdomain = $1 AND status != 'deleted') AS exists`,
      [normalized]
    );
    return !result.rows[0].exists;
  }

  async getProvisioningLogs(tenantId: string): Promise<ProvisioningLog[]> {
    const result = await this.central.query<ProvisioningLog>(
      `SELECT id, step, status, message, started_at, completed_at
  ...
  private async attachTenantIdToLogs(subdomain: string, tenantId: string) {
  ...
  private async startStepLog(context: ProvisioningContext, step: ProvisioningStep, message?: string) {
    const result = await this.central.query<{ id: string }>(
      `INSERT INTO provisioning_logs (tenant_id, subdomain, step, status, message)
       VALUES ($1, $2, $3, 'running', $4)
       RETURNING id`,
      [context.tenantId ?? null, context.subdomain, step, message || null]
    );
    return result.rows[0].id;
  }

  private async finishStepLog(
    logId: string,
    status: Exclude<ProvisioningLogStatus, 'pending'>,
    message?: string,
    ...
    await this.central.query(
      `UPDATE provisioning_logs
       SET status = $2,
           ...
           completed_at = NOW()
       WHERE id = $1`,
      [logId, status, message || null, errorDetails ? JSON.stringify(errorDetails) : null]
    );
  }
  ...
  async logStep(
    ...
    await this.central.query(
      `INSERT INTO provisioning_logs (tenant_id, subdomain, step, status, message, error_details)
       VALUES ($1, $2, $3, $4, $5, $6::jsonb)`,
      [context.tenantId ?? null, context.subdomain, step, status, message || null, errorDetails ? JSON.stringify(errorDetails) : null]
    );
  }
  ...
  private async runStep<T>(
    ...
    step: ProvisioningStep,
    handler: () => Promise<T>,
    messages?: { start?: string; success?: string }
  ): Promise<T> {
    ...
    try {
      ...
      await this.finishStepLog(logId, 'failed', (error as Error)?.message, {
   ...
   */
  private async executeRollbackActions(context: ProvisioningContext): Promise<void> {
  ...
  private async fetchTenantById(id: string): Promise<TenantRow> {
    const result = await this.central.query<TenantRow>(
      `SELECT id, subdomain, company_name, status, subscription_plan, database_url_encrypted, database_name
       FROM tenants
       WHERE id = $1
       LIMIT 1`,
      [id]
  ...
  async getTenantSummary(id: string) {
    const result = await this.central.query(
      `SELECT id, subdomain, company_name, status, subscription_plan, database_name, created_at, suspended_at, deleted_at
       FROM tenants
       WHERE id = $1
       LIMIT 1`,
      [id]
  ...
  async createTenant(input: CreateTenantInput, options: { skipDuplicateCheck?: boolean } = {}): Promise<TenantRow> {
    ...
    try {
      ...
      const result = await client.query<TenantRow>(
        `INSERT INTO tenants (subdomain, company_name, subscription_plan, database_url_encrypted, database_name, status, activated_at)
         ...
         RETURNING id, subdomain, company_name, status, subscription_plan, database_url_encrypted, database_name`,
        [normalizedSubdomain, input.companyName, input.subscriptionPlan, encryptedDbUrl, fallbackDbName]
      );
      ...
      if (input.admin) {
        ...
        await client.query(
          `INSERT INTO tenant_admins (tenant_id, email, password_hash, first_name, last_name, phone, is_primary)
           VALUES ($1, $2, $3, $4, $5, $6, true)
           ON CONFLICT (tenant_id, email) DO NOTHING`,
          [tenant.id, input.admin.email, passwordHash, input.admin.firstName || null, input.admin.lastName || null, input.admin.phone || null]
        );
      }
      ...
      }, (error as Error).message).catch(err => console.error('Failed to create audit log:', err));
  ...
  async runTenantMigrations(tenant: TenantRow, migrationsDir = path.join(process.cwd(), 'migrations', 'tenant')) {
   ...
   */
  private async getAppliedMigrations(pool: Pool): Promise<Set<string>> {
    try {
  ...
  async seedTenantDefaults(tenant: TenantRow) {
   ...
   */
  async createSubscriptionForTenant(tenant: TenantRow): Promise<void> {
    try {
      // Get the plan_id from subscription_plans table
      const planResult = await this.central.query(
      ...
      const priceResult = await this.central.query(
  ...
  async provisioningOrchestrator(
    payload: CreateTenantInput, 
    options: { resumeFromStep?: ProvisioningStep; existingTenantId?: string } = {}
  ) {
    ...
    try {
      ...
      if (!options.resumeFromStep || options.resumeFromStep === 'CREATE_TENANT_RECORD') {
        if (options.existingTenantId) {
          ...
          tenant = await this.runStep(
            context, 
            'CREATE_TENANT_RECORD', 
            () => this.createTenant({ ...payload, subdomain: normalizedSubdomain }),
            {
      ...
      if (payload.admin) {
        if (resumeIndex <= stepOrder.indexOf('CREATE_ADMIN')) {
          await this.runStep(
            context, 
            'CREATE_ADMIN', 
            () => this.createAdminUser(tenant, payload.admin!),
            {
        ...
        if (resumeIndex <= stepOrder.indexOf('SEND_WELCOME_EMAIL')) {
          await this.runStep(
            context, 
            'SEND_WELCOME_EMAIL', 
            () => this.sendWelcomeEmail(tenant, payload.admin!.email),
            {
      ...
      if (context.tenantId) {
        try {
          await this.rollbackProvisioning(context.tenantId, {
            reason: `Provisioning failed at step: ${(error as Error).message}`,
