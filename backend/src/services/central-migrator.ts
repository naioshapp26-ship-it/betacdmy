...
const hasMigrationRun = async (filename: string) => {
  const result = await centralPool.query<{ exists: boolean }>(
    `SELECT EXISTS (SELECT 1 FROM ${MIGRATIONS_TABLE} WHERE filename = $1) AS exists`,
    [filename]
  );
  return result.rows[0]?.exists ?? false;
};

const applyMigration = async (filePath: string, filename: string) => {
