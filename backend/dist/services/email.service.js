...
export class EmailService {
    ...
    async resolveStoredConfig(tenantId) {
        try {
            if (tenantId) {
                const tenantResult = await centralPool.query(`SELECT smtp_host, smtp_port, smtp_user, smtp_pass, smtp_from, smtp_secure
             FROM email_settings
            ...
            const centralResult = await centralPool.query(`SELECT smtp_host, smtp_port, smtp_user, smtp_pass, smtp_from, smtp_secure
           FROM email_settings
...
<body>
  <div class="container">
    ...
    async sendPaymentFailure(options) {
        const { to, tenantName, amount, currency, invoiceNumber, invoiceUrl, nextAttemptDate, attemptCount, } = options;
...
<body>
  <div class="container">
     ...
     */
    async verifyConnection(tenantId) {
        const config = await this.resolveConfig(tenantId);
        if (!config) {
            return {
                success: false,
