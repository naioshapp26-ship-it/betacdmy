...
const extractBlogSlugFromPath = (pathname) => {
    const normalizedPath = normalizeRoutePath(pathname);
    const match = normalizedPath.match(BLOG_PATH_REGEX);
    return match ? match[1] : null;
};
const extractInstructorIdFromPath = (pathname) => {
    const normalizedPath = normalizeRoutePath(pathname);
...
const resolveLocale = (lang, override) => {
    if (override)
        return override;
    return lang === 'ar' ? 'ar_AR' : 'en_US';
};
const resolveAlternateLocale = (lang, override) => {
    if (override)
        return override;
    return lang === 'ar' ? 'en_US' : 'ar_AR';
};
const resolveText = (lang, en, ar) => {
    if (lang === 'ar')
        return ar || en || undefined;
    return en || ar || undefined;
};
const resolveCanonicalUrl = (baseUrl, path, search, override) => {
    if (override)
        return override;
    if (!baseUrl || !path)
        return undefined;
...
const buildDynamicSeo = async (pool, path, search) => {
    const normalizedPath = normalizeRoutePath(path);
    const blogSlug = extractBlogSlugFromPath(normalizedPath);
    if (blogSlug) {
        const result = await pool.query('SELECT title, excerpt, image, uploaded_image_path FROM blog_posts WHERE slug = $1 OR id::text = $1', [blogSlug]);
        if (result.rowCount) {
    ...
    if (normalizedPath === COURSE_PLAYER_PATH) {
        ...
        if (courseId) {
            const result = await pool.query('SELECT title, description, thumbnail FROM courses WHERE id = $1', [courseId]);
            if (result.rowCount) {
                const row = result.rows[0];
                return {
                    contentType: 'course',
    ...
    if (instructorId) {
        const result = await pool.query("SELECT name, bio, avatar FROM users WHERE id = $1 AND role = 'INSTRUCTOR'", [instructorId]);
        if (result.rowCount) {
            const row = result.rows[0];
            return {
...
const mergeSeoRows = (override, base) => {
    if (!override && !base)
        return null;
    const merged = {
    ...
    fields.forEach((field) => {
        ...
        merged[field] = overrideValue !== undefined && overrideValue !== null ? overrideValue : baseValue;
    });
    return merged;
};
const fetchSeoOverride = async (pool, contentType, contentId) => {
    const result = await pool.query('SELECT * FROM seo_overrides WHERE content_type = $1 AND content_id = $2', [contentType, contentId]);
    return result.rowCount ? result.rows[0] : null;
};
export const resolveSeoForPath = async (input) => {
    const { path, search, lang = 'en', tenantPool, isCentral = false, baseUrl, defaults } = input;
    const normalizedPath = normalizeRoutePath(path);
    const pool = isCentral ? null : tenantPool;
    let seoRow = null;
    if (isCentral) {
        const { centralPool } = await import('../central-db.js');
        const result = await centralPool.query('SELECT * FROM central_seo_settings WHERE page_path = $1', [normalizedPath]);
        seoRow = result.rowCount ? result.rows[0] : null;
    }
    else if (pool) {
        const result = await pool.query('SELECT * FROM seo_settings WHERE page_path = $1', [normalizedPath]);
        seoRow = result.rowCount ? result.rows[0] : null;
    }
    ...
    const title = resolveText(lang, effectiveSeo?.title_en, effectiveSeo?.title_ar)
        || dynamicSeo.title
        || defaults?.title;
    const description = resolveText(lang, effectiveSeo?.description_en, effectiveSeo?.description_ar)
        || dynamicSeo.description
        || defaults?.description;
    const keywords = resolveText(lang, effectiveSeo?.keywords_en, effectiveSeo?.keywords_ar)
        || defaults?.keywords;
    const canonical_url = resolveCanonicalUrl(baseUrl, normalizedPath, search, effectiveSeo?.canonical_url);
