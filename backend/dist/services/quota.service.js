import { getPlanQuota } from '../middleware/rate-limiter.js';
export class QuotaService {
    /**
     * Check if tenant has reached user quota
     */
    async checkUserQuota(tenantPool, plan) {
        const quota = getPlanQuota(plan);
        const result = await tenantPool.query('SELECT COUNT(*) as count FROM users');
     ...
     */
    async checkCourseQuota(tenantPool, plan) {
        const quota = getPlanQuota(plan);
        const result = await tenantPool.query('SELECT COUNT(*) as count FROM courses');
        const current = parseInt(result.rows[0]?.count || '0', 10);
     ...
     */
    async checkStorageQuota(tenantPool, plan, additionalSizeGb = 0) {
        const quota = getPlanQuota(plan);
        ...
        const result = await tenantPool.query(`SELECT COALESCE(SUM(file_size_bytes), 0) as total_size 
       FROM uploads 
       WHERE deleted_at IS NULL`).catch(() => ({ rows: [{ total_size: '0' }] }));
     ...
     */
    async getQuotaUsage(tenantPool, plan) {
     ...
     */
    async canPerformOperation(tenantPool, plan, operation, metadata) {
        try {
            let quota;
            switch (operation) {
                case 'add_user':
