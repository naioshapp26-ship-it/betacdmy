...
const isSelfSignedCertificateError = (error) => {
    ...
    if (code && SELF_SIGNED_CODES.has(code))
        return true;
    const message = error?.message?.toLowerCase() ?? '';
    return SELF_SIGNED_KEYWORDS.some((snippet) => message.includes(snippet));
};
const ensurePoolReady = async (tenant, createPool, canRelaxSSL) => {
    let pool = createPool();
    try {
        ...
        if (allowSelfSignedConnections && canRelaxSSL && isSelfSignedCertificateError(error)) {
            console.warn(`[db-manager] Tenant ${tenant.subdomain} presented a self-signed certificate; retrying with relaxed validation.`);
...
const ensurePasswordHashColumn = async (pool) => {
    if (ensuredPasswordHashPools.has(pool))
        return;
    try {
...
const decryptConnectionString = async (tenant) => {
...
export async function getTenantPool(tenant) {
    ...
    const useSSL = tenantSSLOverride
        ...
        : process.env.PGSSL
            ...
            : !isLocal;
    const rejectUnauthorizedEnv = process.env.TENANT_PGSSL_REJECT_UNAUTHORIZED ?? process.env.PGSSL_REJECT_UNAUTHORIZED;
    let rejectUnauthorized = rejectUnauthorizedEnv !== undefined ? rejectUnauthorizedEnv === 'true' : false;
    ...
    const instantiatePool = (overrideReject) => {
        let sslConfig;
        if (useSSL) {
...
export const evictPool = (tenantId) => {
...
export const clearAllPools = async () => {
    await Promise.all(Array.from(pools.values()).map(async (entry) => {
        if (entry.timeout)
            clearTimeout(entry.timeout);
