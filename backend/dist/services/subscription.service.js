...
export class SubscriptionService {
    ...
    async handleSubscriptionUpdated(subscription) {
        ...
        const planResult = await centralPool.query(`SELECT id FROM subscription_plans WHERE code = $1`, [plan]);
        ...
        const existing = await centralPool.query(`SELECT id FROM subscriptions WHERE tenant_id = $1`, [tenantId]);
        ...
        if (existing.rows.length > 0) {
            ...
            await centralPool.query(`UPDATE subscriptions 
         SET plan = $2, 
             plan_id = $3,
             status = $4, 
             price_monthly = $5,
    ...
    async recordInitialPaymentIfMissing(subscription, tenantId, plan, billingCycle) {
    ...
    async getSubscriptionRecordId(tenantId) {
    ...
    async getSubscriptionByTenantId(tenantId) {
        const result = await centralPool.query(`SELECT * FROM subscriptions WHERE tenant_id = $1`, [tenantId]);
    ...
    async cancelSubscription(tenantId, cancelAtPeriodEnd = true) {
        ...
        if (cancelAtPeriodEnd) {
         ...
         WHERE tenant_id = $1`, [tenantId, true]);
        }
        else {
            await centralPool.query(`UPDATE subscriptions 
    ...
    async suspendTenant(tenantId) {
        await centralPool.query(`UPDATE tenants SET status = 'suspended' WHERE id = $1`, [tenantId]);
        console.log(`[SubscriptionService] Suspended tenant ${tenantId}`);
    }
    /**
     * Get Stripe price ID for plan and billing cycle
     */
    getPlanPricing(paymentConfig, plan, billingCycle) {
        const map = {
            basic: {
