...
export class RefundService {
     ...
     */
    async getTenantDatabasePool(tenantId) {
        // Get tenant details from central database
        const result = await centralPool.query('SELECT * FROM tenants WHERE id = $1', [tenantId]);
    ...
    async processRefund(request) {
        ...
        // Get payment details
        const paymentResult = await tenantPool.query(`SELECT * FROM course_payments WHERE id = $1`, [paymentId]);
        if (paymentResult.rows.length === 0) {
            throw new Error('Payment not found');
        }
        const payment = paymentResult.rows[0];
        // Check if payment has already been fully refunded
        const existingRefundsResult = await tenantPool.query(`SELECT COALESCE(SUM(amount), 0) as total_refunded 
       FROM payment_refunds 
       WHERE payment_id = $1 AND status IN ('succeeded', 'pending')`, [paymentId]);
        ...
        const userResult = await tenantPool.query(`SELECT name FROM users WHERE id = $1`, [refundedBy]);
        const refundedByName = userResult.rows[0]?.name || 'Unknown User';
        // Record refund in database
        const refundId = crypto.randomUUID();
        await tenantPool.query(`INSERT INTO payment_refunds (
        ...
        if (totalRefunded + refundAmount >= paymentAmount) {
            await tenantPool.query(`UPDATE course_payments SET notes = COALESCE(notes || ' | ', '') || 'FULLY REFUNDED'
     ...
     */
    async getRefundsForPayment(tenantId, paymentId) {
        const tenantPool = await this.getTenantDatabasePool(tenantId);
        const result = await tenantPool.query(`SELECT * FROM payment_refunds 
       WHERE payment_id = $1 
       ORDER BY refunded_at DESC`, [paymentId]);
     ...
     */
    async getTenantRefunds(tenantId) {
        const tenantPool = await this.getTenantDatabasePool(tenantId);
        const result = await tenantPool.query(`SELECT * FROM payment_refunds 
     ...
     */
    async updateRefundStatus(tenantId, stripeRefundId, status) {
        const tenantPool = await this.getTenantDatabasePool(tenantId);
        const result = await tenantPool.query(`UPDATE payment_refunds 
       SET status = $2 
       WHERE stripe_refund_id = $1
       RETURNING payment_id, amount`, [stripeRefundId, status]);
        if (result.rows.length > 0) {
            ...
            if (status === 'succeeded') {
                const paymentResult = await tenantPool.query(`SELECT amount FROM course_payments WHERE id = $1`, [payment_id]);
                if (paymentResult.rows.length > 0) {
                    const paymentAmount = parseFloat(paymentResult.rows[0].amount);
                    const totalRefundedResult = await tenantPool.query(`SELECT COALESCE(SUM(amount), 0) as total_refunded 
             FROM payment_refunds 
             WHERE payment_id = $1 AND status = 'succeeded'`, [payment_id]);
                    const totalRefunded = parseFloat(totalRefundedResult.rows[0].total_refunded);
                    if (totalRefunded >= paymentAmount) {
                        await tenantPool.query(`UPDATE course_payments 
               SET notes = COALESCE(notes || ' | ', '') || 'FULLY REFUNDED'
               WHERE id = $1`, [payment_id]);
                    }
                }
            }
        }
     ...
     */
    async canRefund(tenantId, paymentId) {
        const tenantPool = await this.getTenantDatabasePool(tenantId);
        // Get payment details
        const paymentResult = await tenantPool.query(`SELECT * FROM course_payments WHERE id = $1`, [paymentId]);
        if (paymentResult.rows.length === 0) {
        ...
        const refundsResult = await tenantPool.query(`SELECT COALESCE(SUM(amount), 0) as total_refunded 
       FROM payment_refunds 
       WHERE payment_id = $1 AND status IN ('succeeded', 'pending')`, [paymentId]);
