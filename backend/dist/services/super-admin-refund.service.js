...
export class SuperAdminRefundService {
    ...
    async getPaymentTransaction(paymentId) {
        const result = await centralPool.query(`SELECT id, tenant_id, subscription_id, amount, currency, status, 
    ...
    async processRefund(request) {
        const { paymentTransactionId, amount, reason, refundedBy, refundedByName, refundedByEmail, tenantId } = request;
        ...
        await centralPool.query(`INSERT INTO subscription_refunds (
       ...
       WHERE id = $3`, [newRefundedAmount, newRefundStatus, paymentTransactionId]);
        // Log to central audit logs
        await centralPool.query(`INSERT INTO audit_logs (
    ...
    async getRefundsForPayment(paymentId) {
        const result = await centralPool.query(`SELECT * FROM subscription_refunds 
    ...
    async getTenantRefunds(tenantId) {
        const result = await centralPool.query(`SELECT sr.*, pt.amount as original_amount
       FROM subscription_refunds sr
       JOIN payment_transactions pt ON sr.payment_transaction_id = pt.id
       WHERE sr.tenant_id = $1 
       ORDER BY sr.refunded_at DESC`, [tenantId]);
        return result.rows.map(row => ({
            id: row.id,
            payment_transaction_id: row.payment_transaction_id,
            tenant_id: row.tenant_id,
    ...
    async getAllRefunds(options) {
        ...
        // Get total count
        const countResult = await centralPool.query(`SELECT COUNT(*) as total FROM subscription_refunds sr ${whereClause}`, values);
        // Get refunds with tenant info
        const result = await centralPool.query(`SELECT sr.*, t.subdomain, t.company_name, pt.amount as original_amount
       FROM subscription_refunds sr
     ...
     */
    async updateRefundStatus(stripeRefundId, status) {
        // Update subscription_refunds table
        const result = await centralPool.query(`UPDATE subscription_refunds 
       SET status = $1, updated_at = NOW()
       WHERE stripe_refund_id = $2
       RETURNING payment_transaction_id, amount, tenant_id`, [status, stripeRefundId]);
        if (result.rows.length === 0) {
        ...
        if (status === 'failed') {
         ...
         SET refunded_amount = refunded_amount - $1,
             refund_status = CASE 
               ...
               ELSE 'partial'
             END
         WHERE id = $2`, [parseFloat(refund.amount), refund.payment_transaction_id]);
        }
