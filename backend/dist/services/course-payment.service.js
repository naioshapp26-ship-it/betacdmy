 ...
 */
export async function resolveTenantRow(tenantId) {
    if (tenantId === 'central') {
        return CENTRAL_VIRTUAL_TENANT;
    }
    const tenantResult = await centralPool.query(`SELECT * FROM tenants WHERE id = $1`, [tenantId]);
 ...
 */
export async function createCourseCheckoutSession(data) {
    const { tenantId, tenantSlug, courseId, studentId, studentEmail, courseName, coursePrice, currency = 'USD' } = data;
    ...
    if (isCentral) {
        ...
        const tenantResult = await centralPool.query(`SELECT id, subdomain, status FROM tenants WHERE id = $1`, [tenantId]);
        if (tenantResult.rows.length === 0) {
            throw new Error('Tenant not found');
        }
 ...
 */
export async function recordCoursePayment(tenant, payment) {
    const pool = await getTenantPool(tenant);
    // Get student and course details
    const studentResult = await pool.query(`SELECT id, name, email FROM users WHERE id = $1`, [payment.userId]);
    const courseResult = await pool.query(`SELECT id, title, instructor FROM courses WHERE id = $1`, [payment.courseId]);
    ...
    const result = await pool.query(`INSERT INTO course_payments 
 ...
 */
export async function enrollStudentInCourse(tenant, data) {
    const pool = await getTenantPool(tenant);
    await pool.query(`
  ...
  `);
    ...
    const existingEnrollment = await pool.query(`SELECT id FROM enrollments WHERE user_id = $1 AND course_id = $2`, [data.userId, data.courseId]);
    if (existingEnrollment.rows.length > 0) {
        console.log(`Student ${data.userId} already enrolled in course ${data.courseId}`);
        return;
    }
    const statusColumn = await pool.query(`SELECT 1
    ...
    if (statusColumn.rowCount) {
        await pool.query(`INSERT INTO enrollments (user_id, course_id, enrolled_at, status)
      VALUES ($1, $2, NOW(), 'active')`, [data.userId, data.courseId]);
    }
    else {
        await pool.query(`INSERT INTO enrollments (user_id, course_id, enrolled_at)
      VALUES ($1, $2, NOW())`, [data.userId, data.courseId]);
    }
    const enrolledCoursesColumn = await pool.query(`SELECT udt_name
     FROM information_schema.columns
     WHERE table_schema = 'public'
    ...
    if (enrolledCoursesColumn.rowCount) {
        const udtName = enrolledCoursesColumn.rows[0]?.udt_name;
        ...
        await pool.query(`UPDATE users
         ...
         END
       WHERE id = $1`, [data.userId, data.courseId]);
    }
 ...
 */
export async function handleCoursePurchaseSuccess(session) {
    const { tenant_id, tenant_slug, course_id, student_id, } = session.metadata || {};
    ...
    if (paymentIntentId) {
        ...
        const chargeId = paymentIntent.latest_charge?.id || paymentIntent.charges?.data?.[0]?.id;
        if (chargeId) {
 ...
 */
export async function confirmCoursePaymentSession(tenant, sessionId) {
    const tenantPool = await getTenantPool(tenant);
    const isCentralTenant = tenant.id === 'central';
    const paymentConfig = isCentralTenant
        ? await paymentConfigService.getCentralPaymentConfig()
 ...
 */
export async function getCoursePaymentBySessionId(tenant, sessionId) {
    const pool = await getTenantPool(tenant);
    const result = await pool.query(`SELECT id, student_id as user_id, course_id, amount, payment_method, 
            received_at as payment_date, receipt_url, stripe_payment_intent_id, stripe_session_id
     FROM course_payments 
     WHERE stripe_session_id = $1`, [sessionId]);
    return result.rows[0] || null;
}
/**
 * Get student's course payments
 */
export async function getStudentCoursePayments(tenant, studentId) {
    const pool = await getTenantPool(tenant);
    const result = await pool.query(`SELECT id, student_id as user_id, course_id, amount, payment_method, 
            received_at as payment_date, receipt_url, stripe_payment_intent_id, stripe_session_id
     FROM course_payments 
