...
import { evictPool } from './db-manager.js';
...
export class TenantCleanupService {
     ...
     */
    async hardDeleteTenant(tenantId, options = {}) {
        const { dropDatabase = true, deleteFiles = true, archiveCentralRecords = true, retentionDays = 90 } = options;
        const result = {
            tenantId,
            subdomain: '',
        ...
        try {
            ...
            try {
                ...
                result.errors.push(`Failed to evict pool: ${error.message}`);
            }
            ...
            if (deleteFiles) {
                try {
                    ...
                    const errorMsg = `Failed to delete files: ${error.message}`;
            ...
            if (dropDatabase && tenant.database_name) {
                try {
                    ...
                    const errorMsg = `Failed to drop database: ${error.message}`;
                    result.errors.push(errorMsg);
                    console.error(`[TenantCleanup] ${errorMsg}`);
                }
            }
            ...
            if (archiveCentralRecords) {
                try {
                    ...
                    const errorMsg = `Failed to archive central records: ${error.message}`;
                    result.errors.push(errorMsg);
                    console.error(`[TenantCleanup] ${errorMsg}`);
                }
            }
                ...
                try {
                    ...
                    const errorMsg = `Failed to delete central records: ${error.message}`;
                    result.errors.push(errorMsg);
                    console.error(`[TenantCleanup] ${errorMsg}`);
                }
            }
     ...
     */
    async deleteTenantFiles(tenant) {
     ...
     */
    async archiveCentralRecords(tenant, retentionDays) {
      ...
      `);
            ...
            const tenantData = await client.query(`SELECT * FROM tenants WHERE id = $1`, [tenant.id]);
     ...
     */
    async deleteCentralRecords(tenantId) {
        const client = await this.central.connect();
        try {
            await client.query('BEGIN');
     ...
     */
    async logCleanupAction(tenantId, action, details) {
        try {
    ...
    async fetchTenantById(id) {
        const result = await this.central.query(`SELECT id, subdomain, company_name, status, subscription_plan, 
     ...
     */
    async getCleanupLogs(tenantId) {
        try {
            const result = await this.central.query(`SELECT id, tenant_id, action, details, created_at
         FROM cleanup_logs
         WHERE tenant_id = $1
