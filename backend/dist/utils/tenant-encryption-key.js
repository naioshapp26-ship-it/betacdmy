 ...
 */
export const resolveTenantDbEncryptionKey = (env = process.env) => {
    ...
    if (env.JWT_SECRET) {
        if (env.NODE_ENV === 'production') {
            console.warn('[tenant-encryption] TENANT_DB_ENCRYPTION_KEY missing; deriving key from JWT_SECRET (set TENANT_DB_ENCRYPTION_KEY explicitly)');
        }
        return `tenant-db:${env.JWT_SECRET}`;
    }
    if (env.NODE_ENV === 'production') {
        throw new Error('TENANT_DB_ENCRYPTION_KEY (or JWT_SECRET) must be configured to encrypt/decrypt tenant databases');
    }
