...
export async function backfillSubscriptions() {
    ...
    try {
        ...
        const tenantsWithoutSubs = await centralPool.query(`SELECT t.id, t.subdomain, t.subscription_plan, t.status
       FROM tenants t
       ...
       ORDER BY t.created_at ASC`);
        ...
        for (const tenant of tenantsWithoutSubs.rows) {
            try {
                ...
                const planResult = await centralPool.query(`SELECT id FROM subscription_plans WHERE code = $1`, [tenant.subscription_plan]);
                ...
                const priceResult = await centralPool.query(`SELECT amount, currency 
           ...
           LIMIT 1`, [planId]);
                ...
                await centralPool.query(`INSERT INTO subscriptions 
           ...
           VALUES ($1, $2, $3, $4, $5, $5, $6, $6, 'monthly', NOW(), NOW() + INTERVAL '1 month', NOW(), NOW())`, [tenant.id, tenant.subscription_plan, planId, tenant.status, lockedAmount, lockedCurrency]);
 ...
 */
export async function validateSubscriptionPricing() {
    const result = {
        total: 0,
        withLockedPricing: 0,
    ...
    try {
        ...
        const subs = await centralPool.query(`SELECT 
         s.id,
         s.tenant_id,
         s.status,
         s.locked_amount,
 ...
 */
export async function getSubscriptionPricingSummary(tenantId) {
    try {
        // Get tenant info
        const tenantResult = await centralPool.query(`SELECT id, subdomain, subscription_plan, status FROM tenants WHERE id = $1`, [tenantId]);
        if (tenantResult.rows.length === 0) {
            throw new Error(`Tenant ${tenantId} not found`);
        }
        const tenant = tenantResult.rows[0];
        // Get subscription
        const subResult = await centralPool.query(`SELECT * FROM subscriptions WHERE tenant_id = $1`, [tenantId]);
        const subscription = subResult.rows[0] || null;
        // Get plan
        const planResult = await centralPool.query(`SELECT * FROM subscription_plans WHERE code = $1`, [tenant.subscription_plan]);
        const plan = planResult.rows[0] || null;
        // Get current price
        const priceResult = plan
            ? await centralPool.query(`SELECT * 
           FROM subscription_plan_prices 
           WHERE plan_id = $1 
             AND billing_cycle = 'monthly'
             AND is_active = true
