...
export const buildPendingDatabaseUrl = (databaseName) => `${PENDING_PLACEHOLDER_PREFIX}${encodeURIComponent(databaseName)}`;
export const isPendingDatabaseUrl = (url) => typeof url === 'string' && url.startsWith(PENDING_PLACEHOLDER_PREFIX);
 ...
 */
export const replaceDatabaseNameInUrl = (connectionString, databaseName) => {
...
export const resolveTenantDatabaseUrlTemplate = (env = process.env) => {
...
export const resolveProvisioningAdminDatabaseUrl = (env = process.env) => {
...
export const quotePgIdentifier = (identifier) => `"${String(identifier).replace(/"/g, '""')}"`;
