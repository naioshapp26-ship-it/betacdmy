...
export const createAIConfigRouter = () => {
     ...
     */
    router.get('/api/super-admin/ai-config', requireAuth, requireRole('super_admin'), async (req, res) => {
        try {
     ...
     * }
     */
    router.put('/api/super-admin/ai-config', requireAuth, requireRole('super_admin'), async (req, res) => {
        try {
            ...
            if (maxTokens !== undefined && (maxTokens < 1 || maxTokens > 100000)) {
                return res.status(400).json(createErrorResponse('errors.invalidMaxTokens', req, 'Max tokens must be between 1 and 100,000'));
            }
            const userId = req.user?.id;
            await aiConfigService.updateAIConfig({ type: 'central' }, {
                aiEnabled,
                aiProvider,
                aiModel,
                apiKey,
     ...
     */
    router.post('/api/super-admin/ai-config/test', requireAuth, requireRole('super_admin'), async (req, res) => {
        try {
            ...
            if (isGeminiProvider(normalizedProvider)) {
                ...
                if (!response.ok) {
                    ...
                    return res.status(400).json(createErrorResponse('errors.aiTestFailed', req, `AI test failed: ${error}`));
                }
     ...
     */
    router.delete('/api/super-admin/ai-config', requireAuth, requireRole('super_admin'), async (req, res) => {
        try {
            const userId = req.user?.id;
            await aiConfigService.updateAIConfig({ type: 'central' }, {
                aiEnabled: false,
