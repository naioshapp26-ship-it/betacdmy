...
export const createTenantPaymentGatewayConfigRouter = () => {
     ...
     */
    router.get('/config', requireTenantPool, requireAuth, requireRole('admin'), async (req, res) => {
        try {
     ...
     * }
     */
    router.put('/config', requireTenantPool, requireAuth, requireRole('admin'), async (req, res) => {
        try {
            const tenantPool = req.tenantPool;
            if (!tenantPool) {
                return res.status(400).json(createErrorResponse('errors.tenantContextRequired', req, 'Tenant context is required'));
            }
            const { stripeEnabled, stripePublicKey, stripeSecretKey, stripeWebhookSecret, paypalEnabled, paypalClientId, paypalSecretKey, visaEnabled, visaPublicKey, visaSecretKey, } = req.body;
            ...
            await paymentConfigService.updatePaymentConfig({ type: 'tenant', tenantPool }, {
                stripeEnabled,
                stripePublicKey,
                stripeSecretKey,
                stripeWebhookSecret,
     ...
     */
    router.post('/test-stripe', requireTenantPool, requireAuth, requireRole('admin'), async (req, res) => {
        try {
     ...
     */
    router.delete('/config/:gateway', requireTenantPool, requireAuth, requireRole('admin'), async (req, res) => {
        try {
            const tenantPool = req.tenantPool;
            if (!tenantPool) {
                return res.status(400).json(createErrorResponse('errors.tenantContextRequired', req, 'Tenant context is required'));
            ...
            await paymentConfigService.updatePaymentConfig({ type: 'tenant', tenantPool }, updates, userId);
            res.json({
                success: true,
