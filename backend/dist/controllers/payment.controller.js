...
export const createPaymentRouter = () => {
    ...
    router.post('/api/payment/checkout', async (req, res) => {
        try {
            ...
            const tenantResult = await centralPool.query(`SELECT id, subdomain FROM tenants WHERE id = $1`, [tenantId]);
     ...
     */
    router.get('/api/payment/subscription/:tenantId', async (req, res) => {
        try {
     ...
     */
    router.post('/api/payment/subscription/:tenantId/cancel', async (req, res) => {
        try {
            const tenantId = getSingleParam(req.params.tenantId);
     ...
     */
    router.get('/api/payment/transactions/:tenantId', async (req, res) => {
        try {
            const tenantId = getSingleParam(req.params.tenantId);
            if (!tenantId) {
                return res.status(400).json({ error: 'Missing tenantId parameter' });
     ...
     */
    router.get('/api/payment/revenue/:tenantId', async (req, res) => {
        try {
            const tenantId = getSingleParam(req.params.tenantId);
            if (!tenantId) {
                return res.status(400).json({ error: 'Missing tenantId parameter' });
     ...
     */
    router.get('/api/payment/health', async (req, res) => {
        res.json({
            status: 'ok',
            service: 'payment',
