...
export const createTenantRouter = (provisioning = new ProvisioningService()) => {
    ...
    router.post('/api/payment/tenant-signup/checkout', async (req, res) => {
        try {
            ...
            if (!tenantId || !gateway || !plan || !customerEmail || !subdomain) {
                return res.status(400).json(createErrorResponse('errors.validationRequired', req, 'Missing required fields: tenantId, gateway, plan, customerEmail, subdomain'));
            }
            // Validate gateway
            if (!['stripe', 'paypal'].includes(gateway)) {
                return res.status(400).json(createErrorResponse('errors.invalidGateway', req, 'Invalid payment gateway. Must be stripe or paypal'));
            }
            // Validate plan
            if (!['basic', 'pro', 'enterprise'].includes(plan)) {
                return res.status(400).json(createErrorResponse('errors.validationRequired', req, 'Invalid plan. Must be basic, pro, or enterprise'));
            }
            // Validate billing cycle
            if (!['monthly', 'yearly'].includes(billingCycle)) {
                return res.status(400).json(createErrorResponse('errors.validationRequired', req, 'Invalid billing cycle. Must be monthly or yearly'));
            }
            // Verify tenant exists and is in pending_payment status
            const tenantResult = await provisioning['central'].query(`SELECT id, subdomain, status FROM tenants WHERE id = $1`, [tenantId]);
            ...
            if (tenant.status !== 'pending_payment' && tenant.status !== 'active') {
                return res.status(400).json(createErrorResponse('errors.validationRequired', req, `Tenant status must be pending_payment or active, current status: ${tenant.status}`));
            }
            ...
            return res.status(500).json(createErrorResponse('errors.paymentFailed', req, error.message || 'Failed to create checkout session'));
        }
    });
    /**
    ...
    router.post('/api/payment/tenant-signup/confirm', async (req, res) => {
        try {
            ...
            if (!tenantId || !sessionId) {
                return res.status(400).json(createErrorResponse('errors.validationRequired', req, 'Missing required fields: tenantId, sessionId'));
            }
            ...
            if (result.status !== 'paid') {
                return res.status(400).json(createErrorResponse('errors.paymentFailed', req, 'Payment not completed'));
            }
            return res.json({
                success: true,
            ...
            return res.status(500).json(createErrorResponse('errors.paymentFailed', req, error.message || 'Failed to confirm payment'));
        }
    });
    ...
    router.post('/api/tenants/:tenantId/activate-payment', async (req, res) => {
        try {
            ...
            if (tenant.status === 'pending_payment') {
                await provisioning['central'].query(`UPDATE tenants SET status = 'active', updated_at = NOW() WHERE id = $1`, [tenantId]);
            ...
            return res.status(400).json(createErrorResponse('errors.validationRequired', req, `Cannot activate tenant with status: ${tenant.status}`));
        }
        catch (error) {
            console.error('Tenant activation failed', error);
            return res.status(500).json(createErrorResponse('errors.apiServerError', req, error.message || 'Failed to activate tenant'));
        }
    });
    router.get('/api/subdomains/check', async (req, res) => {
        try {
            const candidate = (req.query.subdomain || req.query.value || '').trim().toLowerCase();
            if (!candidate) {
    ...
    router.post('/api/provisioning/resume/:tenantId', async (req, res) => {
        try {
            ...
            res.status(500).json(createErrorResponse('errors.apiServerError', req, error.message));
        }
    });
    ...
    router.patch('/api/admin/tenants/:id/suspend', async (req, res) => {
        try {
            const { id } = req.params;
            await provisioning['central'].query(`UPDATE tenants SET status = 'suspended', suspended_at = now() WHERE id = $1`, [id]);
    ...
    router.patch('/api/admin/tenants/:id/reactivate', async (req, res) => {
        try {
            const { id } = req.params;
            await provisioning['central'].query(`UPDATE tenants SET status = 'active', suspended_at = NULL WHERE id = $1`, [id]);
    ...
    router.delete('/api/admin/tenants/:id', async (req, res) => {
        try {
            ...
            await provisioning['central'].query(`UPDATE tenants SET status = 'deleted', deleted_at = now() WHERE id = $1`, [id]);
            res.status(204).send();
        }
        catch (error) {
