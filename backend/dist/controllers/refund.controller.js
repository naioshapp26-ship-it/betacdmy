...
export const createRefundRouter = () => {
     ...
     * Requires admin role
     */
    router.post('/api/refunds/:paymentId', requireTenantPool, requireRole('ADMIN'), async (req, res) => {
        try {
            ...
            if (!paymentId) {
                return res.status(400).json(createErrorResponse('errors.invalidInput', req, 'Payment ID is required'));
            }
            if (!tenantId || !userId) {
                return res.status(401).json(createErrorResponse('errors.unauthorized', req, 'Authentication required'));
            }
            // Validate amount if provided (for partial refunds)
            if (amount !== undefined) {
                const numAmount = parseFloat(amount);
                if (isNaN(numAmount) || numAmount <= 0) {
                    return res.status(400).json(createErrorResponse('errors.invalidAmount', req, 'Invalid refund amount'));
                }
            }
            // Check if payment can be refunded
            const canRefundResult = await refundService.canRefund(tenantId, paymentId);
            if (!canRefundResult.canRefund) {
                return res.status(400).json(createErrorResponse('errors.refundNotAllowed', req, canRefundResult.reason || 'Refund not allowed'));
            }
            // Process the refund
            ...
            res.status(statusCode).json(createErrorResponse('errors.paymentRefundFailed', req, errorMessage));
        }
    });
    /**
     ...
     */
    router.get('/api/refunds/payment/:paymentId', requireTenantPool, requireRole('ADMIN'), async (req, res) => {
        try {
            const paymentId = getSingleParam(req.params.paymentId);
            const tenantId = req.tenant?.id;
            if (!paymentId) {
            ...
            res.status(500).json(createErrorResponse('errors.refundQueryFailed', req, error.message || 'Failed to retrieve refunds'));
        }
    });
    /**
     * GET /api/refunds
     ...
     */
    router.get('/api/refunds', requireTenantPool, requireRole('ADMIN'), async (req, res) => {
        try {
            const tenantId = req.tenant?.id;
            if (!tenantId) {
                return res.status(401).json(createErrorResponse('errors.unauthorized', req, 'Authentication required'));
     ...
     */
    router.get('/api/refunds/check/:paymentId', requireTenantPool, requireRole('ADMIN'), async (req, res) => {
        try {
            const paymentId = getSingleParam(req.params.paymentId);
            const tenantId = req.tenant?.id;
            if (!paymentId) {
            ...
            res.status(500).json(createErrorResponse('errors.refundCheckFailed', req, error.message || 'Failed to check refund eligibility'));
        }
    });
    return router;
};
