 ...
 */
export const requireTenantPool = (req, res, next) => {
    ...
    if (!tenantPool) {
        ...
        return res.status(500).json(createErrorResponse('errors.tenantPoolMissing', req, 'Tenant database pool not available. This is a security violation.'));
    }
    if (!tenant) {
        ...
        return res.status(404).json(createErrorResponse('errors.tenantNotFound', req, 'Tenant context not found'));
    }
    ...
    if (tenant.status !== 'active' && tenant.status !== 'pending_payment') {
        ...
        if (tenant.status === 'suspended') {
            return res.status(403).json(createErrorResponse('errors.tenantSuspended', req, 'This tenant has been suspended'));
        }
        return res.status(403).json(createErrorResponse('errors.tenantInactive', req, 'Tenant is not active'));
    }
 ...
 */
export const preventTenantPoolAccess = (req, res, next) => {
    const tenantPool = req.tenantPool;
    if (tenantPool) {
 ...
 */
export const blockSuperAdminOnTenant = (req, res, next) => {
 ...
 */
export const isTenantSubdomain = (req) => {
 ...
 */
export const validateQuerySafety = (req, res, next) => {
    const body = req.body;
