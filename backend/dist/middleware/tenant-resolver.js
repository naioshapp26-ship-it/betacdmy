 ...
 */
const sendTenantErrorPage = (res, statusCode, title, message, additionalInfo) => {
    const html = `
<!DOCTYPE html>
 ...
 */
export const resolveTenantSlug = (req) => {
...
const attachTenantContext = async (req, res, { requireTenant = true } = {}) => {
    const effectiveHost = getEffectiveHost(req);
    ...
    if (!subdomain && !normalizedHost) {
        if (requireTenant) {
            if (isApiRequest(req)) {
                ...
                sendTenantErrorPage(res, 404, 'الموقع غير متاح', 'هذا الموقع غير متاح حالياً.');
            }
            return false;
        }
        return true;
    ...
    if (subdomain && !isValidSubdomain(subdomain)) {
        if (isApiRequest(req)) {
            ...
            sendTenantErrorPage(res, 400, 'عنوان الموقع غير صالح', 'عنوان هذا الموقع غير صحيح.');
        }
        return false;
    }
    try {
        const result = await centralPool.query(`SELECT id, subdomain, company_name, status, subscription_plan, database_url_encrypted, database_name, settings, custom_domain
         FROM tenants
         ...
         LIMIT 1`, [subdomain, normalizedHost, hostWithWww]);
        if (!result.rowCount) {
            // Explicit tenant slug via /t/{sub}, header, or cookie must not silently fall back to main site.
            const explicitTenantRequested = Boolean(pathSubdomain || req.headers[TENANT_SUBDOMAIN_HEADER]);
        ...
        if (tenant.status === 'suspended') {
            if (isApiRequest(req)) {
                ...
                sendTenantErrorPage(res, 403, 'الموقع معلق مؤقتاً', 'هذا الموقع غير متاح حالياً.');
            }
            return false;
        }
        req.tenant = {
            ...
            ...tenant?.custom_domain
                ? { custom_domain: normalizeCustomDomain(tenant.custom_domain) }
                : {}
        };
        ...
        if (isApiRequest(req)) {
            ...
            sendTenantErrorPage(res, 500, 'حدث خطأ ما', 'واجهنا خطأ أثناء تحميل هذا الموقع.');
        }
        return false;
    }
};
...
export const optionalTenantResolver = async (req, res, next) => {
