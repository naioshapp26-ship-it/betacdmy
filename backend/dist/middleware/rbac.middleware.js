...
export const requirePermission = (permissionName) => {
    return async (req, res, next) => {
        ...
        try {
            const hasPermission = await RBACService.userHasPermission(tenantPool, userId, permissionName);
            if (!hasPermission) {
                const lang = req.headers['accept-language']?.includes('ar') ? 'ar' : 'en';
                const message = lang === 'ar'
                ...
                return res.status(403).json(createErrorResponse('errors.permissionDenied', req, message));
            }
...
export const requireAnyPermission = (permissionNames) => {
    return async (req, res, next) => {
        ...
        if (!tenantPool) {
            return res.status(500).json(createErrorResponse('errors.tenantPoolMissing', req, 'Tenant context missing'));
        }
        try {
            const hasPermission = await RBACService.userHasAnyPermission(tenantPool, userId, permissionNames);
            if (!hasPermission) {
                ...
                return res.status(403).json(createErrorResponse('errors.permissionDenied', req, 'Insufficient permissions'));
            }
            next();
        }
        catch (error) {
...
export const requireAllPermissions = (permissionNames) => {
    return async (req, res, next) => {
        ...
        try {
            const hasPermissions = await RBACService.userHasAllPermissions(tenantPool, userId, permissionNames);
            if (!hasPermissions) {
...
export const requireRole = (roleName) => {
    return async (req, res, next) => {
        ...
        if (roleName.toLowerCase() === 'super_admin' && isTenantSubdomain(req)) {
            ...
            return res.status(403).json(createErrorResponse('errors.superAdminNotOnTenant', req, message));
        }
        if (!userId) {
            const lang = req.headers['accept-language']?.includes('ar') ? 'ar' : 'en';
            const message = lang === 'ar'
        ...
        try {
            ...
            return res.status(500).json(createErrorResponse('errors.apiServerError', req, 'Role check failed'));
        }
    };
};
/**
 ...
 */
export const checkPermission = async (req, permissionName) => {
    const userId = req.userId;
    const tenantPool = req.tenantPool;
    if (!userId || !tenantPool) {
        return false;
 ...
 */
export const requireOwnershipOrPermission = (ownerIdField, permissionName) => {
    return async (req, res, next) => {
        const userId = req.userId;
        const tenantPool = req.tenantPool;
        ...
        try {
            ...
            if (!hasPermission) {
                ...
                return res.status(403).json(createErrorResponse('errors.permissionDenied', req, 'Access denied: not owner and insufficient permissions'));
            }
            next();
        }
        catch (error) {
            console.error('[RBAC] Ownership/permission check failed', error);
            return res.status(500).json(createErrorResponse('errors.apiServerError', req, 'Authorization check failed'));
        }
    };
};
/**
...
export const isAdmin = async (req) => {
    ...
    try {
        ...
        if (hasAdminRole)
            return true;
        const hasAdminPermission = await RBACService.userHasAnyPermission(tenantPool, userId, ['admin:*', 'admin:full']);
 ...
 */
export const canAccessResource = async (req, resourceOwnerId, permissionName) => {
    const userId = req.userId;
    const tenantPool = req.tenantPool;
    if (!userId)
        return false;
